...
# 胶片系统
def get_film_session(hostname='10.8.200.24',dbname='at_pacsdb',user='guest',passwd='guest',port=3306,echo=False):
    '''
    :param hostname: 主机
    :param dbname: 数据库
    :param user: 用户
    ...
    :return:
    '''
    engine_str = 'mysql+mysqldb://%s:%s@%s:%s/%s?charset=utf8' %(user,passwd,hostname,port,dbname)
    engine = create_engine(engine_str,echo=False)
    session = sessionmaker(bind=engine)
    return session()
