...
class AchievementsDoctor(Widget):

    def __init__(self,parent=None):
        super(AchievementsDoctor,self).__init__(parent)
        self.setParas()
        self.initUI()
        # 绑定信号槽
    ...
    def setParas(self):
        results = self.session.query(MT_GY_DMZD).filter(MT_GY_DMZD.dmlb=='1229',MT_GY_DMZD.dmsb>0).all()
        self.doctors = [str2(result.dmmc) for result in results]
    ...
    def on_btn_query_click(self):
        ...
        if self.cb_user=='所有':
            if self.login_name in self.doctors:
                pass
            else:
                mes_about(self,"您没有查看他人绩效的权限！")
                return
        else:
            # 查看自己的绩效
            pass
